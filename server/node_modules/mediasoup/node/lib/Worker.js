...
const FbsNotification = __importStar(require("./fbs/notification"));
...
class WorkerImpl extends enhancedEvents_1.EnhancedEventEmitter {
    ...
    constructor({ logLevel, logTags, rtcMinPort, rtcMaxPort, dtlsCertificateFile, dtlsPrivateKeyFile, workerBin, libwebrtcFieldTrials, disableLiburing, appData, }) {
        ...
        let spawnArgs = [];
        if (node_process_1.default.env['MEDIASOUP_USE_VALGRIND'] === 'true') {
            spawnBin = node_process_1.default.env['MEDIASOUP_VALGRIND_BIN'] ?? 'valgrind';
            if (node_process_1.default.env['MEDIASOUP_VALGRIND_OPTIONS']) {
                spawnArgs = spawnArgs.concat(node_process_1.default.env['MEDIASOUP_VALGRIND_OPTIONS'].split(/\s+/));
            }
        ...
        const processListenerCount = node_process_1.default.getMaxListeners();
        if (processListenerCount >= 10) {
            node_process_1.default.setMaxListeners(processListenerCount + 2);
        }
        node_process_1.default.once('SIGINT', this.onSignal);
        node_process_1.default.once('SIGTERM', this.onSignal);
    ...
    close() {
        ...
        logger.debug('close()');
        this.#closed = true;
        node_process_1.default.removeListener('SIGINT', this.onSignal);
        node_process_1.default.removeListener('SIGTERM', this.onSignal);
        // Close every Router.
...
function getDefaultWorkerBin() {
    ...
    if (node_process_1.default.env['MEDIASOUP_WORKER_BIN']) {
        logger.debug(`getDefaultWorkerBin() | using MEDIASOUP_WORKER_BIN environment variable: ${node_process_1.default.env['MEDIASOUP_WORKER_BIN']}`);
        return node_process_1.default.env['MEDIASOUP_WORKER_BIN'];
    }
    ...
    // the Release binary.
    const buildType = node_process_1.default.env['MEDIASOUP_BUILDTYPE'] === 'Debug' ? 'Debug' : 'Release';
